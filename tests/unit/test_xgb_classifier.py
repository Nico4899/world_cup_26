...
def _synthetic_corpus(n: int = 600, seed: int = 0) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    # Venue altitude + travel-km diff + log-market-value diff — included
    # as low-signal columns so XGB tests stay forward-compatible with the
    # Wave 2 DEFAULT_FEATURE_COLUMNS expansion.
    venue_altitude_m = rng.uniform(0.0, 2500.0, size=n)
    travel_km_diff = rng.normal(0.0, 1500.0, size=n)
    log_market_value_diff = rng.normal(0.0, 1.0, size=n)
    X = pd.DataFrame(
        {
            "elo_diff": elo_diff,
            ...
            "log_market_value_diff": log_market_value_diff,
        }
    )
    return X, y
